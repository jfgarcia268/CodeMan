...
- **`defaultChildOrder` (tree.js) must reproduce `buildTree`'s comparator EXACTLY — pinned by a
  ...
  *"non-default"* verdict writes an order array that reproduces the same visible order (harmless); a
  ...
  for two siblings differing solely in case, impossible on a case-insensitive volume, and exists to
  make the JS side a total order, not to chase that case. `decorateChild` runs `TextEncoder` **O(n)**,
  not O(n log n). **The two comparators are in different languages and cannot call each other**, so
  both are pinned to ONE hard-coded oracle string `SORT_ORACLE_JSON` (an adversarial 18-name corpus:
  ...
  install an order file and defeat the point) — and a **12th CI invariant** extracts the literal from
